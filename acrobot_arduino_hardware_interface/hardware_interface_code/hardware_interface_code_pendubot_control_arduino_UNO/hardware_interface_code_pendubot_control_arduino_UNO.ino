...
#define ENC_PHASE_A 3
#define ENC_PHASE_B 8
...
void setup() {
  ...
  pinMode(ENC_PHASE_A, INPUT_PULLUP);
  pinMode(ENC_PHASE_B, INPUT_PULLUP);
  
  // Attach interrupt to the encoder A pin
  attachInterrupt(digitalPinToInterrupt(ENC_A_PIN), encoderISR, RISING);
  attachInterrupt(digitalPinToInterrupt(ENC_PHASE_A), opticalencoderISR, RISING);
...
void loop()
{
    ...
    while(Serial.available() == 0) {
      // Wait for data
      //Serial.println(OpticalEncoderCount);
    }
